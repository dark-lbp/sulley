...
class request (pgraph.node):
    ...
    def __init__(self, name):
        ...
        Can hold any block structure or primitive. This can essentially be
        thought of as a super-block, root-block, daddy-block or whatever other
        alias you prefer.
        ...
        self.block_stack = []       # list of open blocks,
        self.closed_blocks = {}     # dictionary of closed blocks.
        self.callbacks = {}         # dictionary of list of sizers / checksums
        self.names = {}             # dictionary of directly accessible
                                    # primitives.
    ...
    def push(self, item):
        '''
        Push an item into the block structure. If no block is open, the item
        goes onto the request stack. otherwise, the item goes onto the last
        open blocks stack.
        '''
        ...
        # if there are no open blocks, the item gets pushed onto the request
        # stack. Otherwise, the pushed item goes onto the stack of the last
        # opened block.
        if not self.block_stack:
    ...
    def walk(self, stack=None):
        '''
        Recursively walk through and yield every primitive and block on the
        request stack.
...
class block:
    ...
    def __init__(self, name, request, group=None, encoder=None, dep=None,
                 dep_value=None, dep_values=[], dep_compare="=="):
        '''
        The basic building block. Can contain primitives, sizers, checksums or
        other blocks.
        ...
        @type  group:       String
        @param group:       (Optional, def=None) Name of group to associate
        this block with
        @type  encoder:     Function Pointer
        @param encoder:     (Optional, def=None) Optional pointer to a
        function to pass rendered data to prior to return
        @type  dep:         String
        @param dep:         (Optional, def=None) Optional primitive whose
        specific value this block is dependant on
        @type  dep_value:   Mixed
        @param dep_value:   (Optional, def=None) Value that field "dep" must
        contain for block to be rendered
        @type  dep_values:  List of Mixed Types
        @param dep_values:  (Optional, def=[]) Values that field "dep" may
        contain for block to be rendered
        @type  dep_compare: String
        @param dep_compare: (Optional, def="==") Comparison method to apply to
        dependency (==, !=, >, >=, <, <=)
        '''
        ...
        self.fuzzable = True    # blocks are always fuzzable because they may
                                # contain fuzzable items.
        self.group_idx = 0      # if this block is tied to a group, the index
                                # within that group.
        self.fuzz_complete = False  # whether or not we are done fuzzing this
                                    # block.
        self.mutant_index = 0       # current mutation index.
    ...
    def mutate(self):
        ...
        if self.group:
            ...
            if not mutated:
                ...
                # if the group values are exhausted, we are done with this
                # block.
                ...
                # otherwise continue mutating this group/block.
                    ...
                    # this the mutate state for every item in this blocks
                    # stack. NOT THE BLOCK ITSELF THOUGH! (hence why we didn't
                    # call self.reset())
                    for item in self.stack:
                        if item.fuzzable:
                            item.reset()

                    # now mutate the first field in this block before
                    # continuing (we repeat a test case if we don't mutate
                    # something)
                    for item in self.stack:
                        if item.fuzzable and item.mutate():
                            mutated = True
        ...
        # if this block is dependant on another field, then manually update
        # that fields value appropriately while we mutate this block. we'll
        # restore the original value of the field prior to continuing.
        if mutated and self.dep:
            # if a list of values was specified, use the first item in the
            # list.
            if self.dep_values:
                self.request.names[self.dep].value = self.dep_values[0]

            # if a list of values was not specified, assume a single value is
            # present.
            else:
    ...
    def num_mutations(self):
        ...
        # if this block is associated with a group, then multiply out the
    ...
    def render(self):
        ...
        # if this block is dependant on another field and the value is not
        # met, render nothing.
        if self.dep:
            if self.dep_compare == "==":
        ...
        # the block is now closed, clear out all the entries from the request
        # back splice dictionary.
    ...
    def reset(self):
        '''
        Reset the primitives on this blocks stack to the starting mutation
        state.
        '''
...
class checksum:
    ...
    def __init__(self, block_name, request, algorithm="crc32", length=0,
        ...
        You *can not* create a checksm for any
        ...
        @param algorithm:  (Optional, def=crc32) Checksum algorithm to use.
        (crc32, adler32, md5, sha1)
        @type  length:     Integer
        @param length:     (Optional, def=0) Length of checksum, specify 0 to
        auto-calculate
        @type  endian:     Character
        @param endian:     (Optional, def=LITTLE_ENDIAN) Endianess of the bit
        field (LITTLE_ENDIAN: <, BIG_ENDIAN: >)
        @type  name:       String
        ...
        if not self.length and self.algorithm not in self.checksum_lengths:
            self.length = self.checksum_lengths[self.algorithm]

    def checksum(self, data):
        '''
        Calculate and return the checksum (in raw bytes) over the supplied
        data.
    ...
    def render(self):
        '''
        Calculate the checksum of the specified block using the specified
        algorithm.
        '''

        self.rendered = ""

        # if the target block for this sizer is already closed, render the
        # checksum.
...
class repeat:
    ...
    '''
    This block type is kind of special in that it is a hybrid between a block
    and a primitive (it can be fuzzed). The user does not need to be wary of
    this fact.
    '''
    ...
    def __init__(self, block_name, request, min_reps=0, max_reps=None, step=1,
        ...
        This block modifier MUST come after the block it is being applied to.
        ...
        @param min_reps:   (Optional, def=0) Minimum number of block
        repetitions
        @type  max_reps:   Integer
        @param max_reps:   (Optional, def=None) Maximum number of block
        repetitions
        @type  step:       Integer
        @param step:       (Optional, def=1) Step count between min and max
        reps
        @type  variable:   Sulley Integer Primitive
        @param variable:   (Optional, def=None) Repititions will be derived
        from this variable, disables fuzzing
        @type  fuzzable:   Boolean
        @param fuzzable:   (Optional, def=True) Enable/disable fuzzing of this
        primitive
        @type  name:       String
        @param name:       (Optional, def=None) Specifying a name gives you
        direct access to a primitive
        '''
        ...
        self.fuzz_complete = False              # flag if this primitive has
                                                # been completely fuzzed
        self.fuzz_library = []                  # library of static fuzz
                                                # heuristics to cycle through.
        ...
        # ensure the user specified either a variable to tie this repeater to
        # or a min/max val.
        ...
        # if not binding variable was specified, propogate the fuzz library
        # with the repetition counts.
        if not self.variable:
            self.fuzz_library = range(self.min_reps,
                                      self.max_reps + 1,
                                      self.step)
        # otherwise, disable fuzzing as the repitition count is determined by
        # the variable.
        else:
            self.fuzzable = False
    ...
    def mutate(self):
        '''
        Mutate the primitive by stepping through the fuzz library, return
        False on completion. If variable-bounding is specified then fuzzing is
        implicitly disabled. Instead, the render() routine will properly
        calculate the correct repitition and return the appropriate data.
        ...
        # if fuzzing was disabled or complete, and mutate() is called, ensure
        ...
        # set the current value as a multiple of the rendered block based on
        # the current fuzz library count.
    ...
    def render(self):
        ...
        # if a variable-bounding was specified then set the value
        # appropriately.
        if self.variable:
            block = self.request.closed_blocks[self.block_name]
...
class size:
    ...
    def __init__(self, block_name, request, length=4, endian="<",
                 format="binary", inclusive=False, signed=False, math=None,
        ...
        *can not* create a sizer for any currently open blocks.
        ...
        field (LITTLE_ENDIAN: <, BIG_ENDIAN: >)
        @type  format:     String
        @param format:     (Optional, def=binary) Output format, "binary" or
        "ascii"
        @type  inclusive:  Boolean
        @param inclusive:  (Optional, def=False) Should the sizer count its
        own length?
        @type  signed:     Boolean
        @param signed:     (Optional, def=False) Make size signed vs. unsigned
        (applicable only with format="ascii")
        @type  math:       Function
        @param math:       (Optional, def=None) Apply the mathematical
        operations defined in this function to the size
        @type  fuzzable:   Boolean
        @param fuzzable:   (Optional, def=False) Enable/disable fuzzing of
        this sizer
        @type  name:       String
        ...
        self.bit_field = primitives.bit_field(0, self.length*8,
                                              endian=self.endian,
                                              format=self.format,
                                              signed=self.signed)
        self.rendered = ""
    ...
    def render(self):
        ...
        # if the sizer is fuzzable and we have not yet exhausted the the
        # possible bit field values, use the fuzz value.
